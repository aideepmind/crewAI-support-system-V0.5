...
from support.customer_support.flow.FitnessConsultingProcessFlow import (
    FitnessConsultingProcessFlow,
)
...
class MainFlow(Flow[SupportState]):
    ...
    async def route_query(self):
        ...
        required_keys = (
            required_data.keys() if isinstance(required_data, dict) else required_data
        )
        ...
        if missing_slots:
            ...
            print(
                f"==== 校验未通过: 意图 [{self.state.intent}] 缺少关键槽位 {missing_slots}，跳转到追问节点"
            )
    ...
    def handle_missing_data(self):
        ...
        if isinstance(required_map, list):
            # 如果是列表，将其转为简单的字典映射
            required_map = {
                slot: f"请补充您的【{slot}】信息。" for slot in required_map
            }
    ...
    async def run_faultDiagnosisProcess(self):
        ...
        print(
            f"++++++开始/继续 故障诊断逻辑。当前轨迹长度: {len(self.state.diagnose_trace)}"
        )
        ...
        if self.state.user_response:
            print(
                f"+++++故障针对槽位信息及用户输入信息self.state.user_response:{self.state.user_response}"
            )
        ...
        if not self.state.slots.get("last_asked_question"):
            ...
            symptom_topic = llm.invoke(
                constants.GET_symptom_desc + self.state.query
            ).content
            ...
            case1: FaultCase = generate_fault_case_from_faultlist_obj(
                fault_list, symptom_topic
            )
        ...
        ## 如果不是第一次提问，从state中获得list和case，而不是创建新的信息
            ...
            if curq.type == "symptoms":
                result = CommonIntentCheck.evaluate(
                    ...
                    user_input,
                    "",
                    constants.Symptom_answer_check,
                    ismock=False,
                )
    ...
    async def run_fitnessConsultingProcess(self):
        ...
        print(
            f">>>>>fitnessConsulting子流程返回给主流程的信息: {sub_flow.state.response}"
        )
    ...
    async def _execute_routing_crew(self) -> IntentOutput:
        ...
        routing_task = self.tasks_factory.routing_task(
            r_agent, self.state.query, self.state.intent, self.state.slots, last_q
        )
        result = Crew(agents=[r_agent], tasks=[routing_task], verbose=False).kickoff()
                ...
                for pattern in json_patterns:
                    ...
                    if match:
                        try:
                            ...
                            if '"intent"' in potential_json:
                                raw_output = json.loads(potential_json)
                                break
                        except json.JSONDecodeError:
                            continue
                else:
                    raw_output = {
                        ...
                        "slots": {},
                        "is_complete": False,
                        "explanation": raw_str[:500] if raw_str else "解析失败",
                    }

        return IntentOutput(
            intent=raw_output.get("intent", "unknown")
            if isinstance(raw_output, dict)
            else "unknown",
            slots=raw_output.get("slots", {}) if isinstance(raw_output, dict) else {},
            is_complete=raw_output.get("is_complete", False)
            if isinstance(raw_output, dict)
            else False,
            explanation=raw_output.get("explanation", raw_str[:200])
            if isinstance(raw_output, dict)
            else raw_str[:200],
        )
